...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int, 
        default=100, 
        help="Save progress to output file every N matches (default: 100)"
    )
    parser.add_argument(
        "--limit", 
        type=int, 
...
def main():
    ...
    try:
        for idx, mid in enumerate(missing_ids):
            ...
            processed_count += 1
            # Save progress periodically (default: every 100 matches)
            if processed_count % args.save_interval == 0:
                print(f" Saving progress (Interval)... ", end="", flush=True)
    ...
    # Save final progress
    print(f"\nSaving final progress to '{args.output}'...")
    save_details(args.output, details_dict)
    print("Saved.")
